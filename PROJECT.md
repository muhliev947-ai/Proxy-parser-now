...
**51 тест** распределены по четырём файлам:
...
1. **Линтеры и тесты** — `ruff check` и `pytest` возвращают 0 ошибок (51 тест).
...
6. **Свежесть узлов** — бесплатные прокси быстро устаревают; даже из 1000+ распарсенных кандидатов REAL-проверку через Hiddify проходят единицы. Это не баг парсера, а природа публичных подписок: `include_unchecked: false` гарантирует, что в подписку попадают только реально рабочие узлы
...
| Метрика | Значение |
|---------|----------|
| Тестов | 51 (26 parser + 10 checker + 7 filters + 8 generator) |
