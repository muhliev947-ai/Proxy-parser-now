...
- `min_working_nodes` — явный параметр в `config.yaml` (постоянное значение **10** с 2026-09-24; при тест-прогонах новых источников временно поднимался до 30). В коде дефолт `0` (= без ранней остановки), поэтому значение всегда читать из конфига
- **⚠️ Известное ограничение batch-цикла (min_working_nodes):** batch-цикл в `_check_with_pool` останавливается сразу, как только `verified >= min_working_nodes` **в середине итерации**. Источники, стоящие в конце списка `sources`, могут получить заниженную долю `checked` в `by_source` (а при `min_working_nodes ≥ 30` — нулевую, т.к. не дойдут до своего батча). `by_source` в `metrics.json` — это доля *проверенных* по источникам, а не по всём *доступным* кандидатам. Для честной оценки нового/позднего источника временно поднимайте `min_working_nodes` (пока не станет 0 или кандидаты не кончатся) — не судьте по одному прогону с низким порогом.
